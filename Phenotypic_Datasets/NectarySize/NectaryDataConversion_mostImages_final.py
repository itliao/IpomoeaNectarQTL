...
# combined most files BUT one (because same image numbers used) 
# see NectaryDataConversion_for1file_final.py
...
#path to working directory of files to merge
os.chdir("/Users/irene/Dropbox (Duke Bio_Ea)")
os.chdir("Rausher Lab/Current_lab_members/Irene_Liao_Data/Lab/F5_RILs/NectaryPhotos-measurement/measurements/to_merge")
...
#create new dataframe for merged files
allXY = pd.DataFrame(columns = ["Label", "X", "Y"])


#read in all files, add first 3 columns to a large combined dataframe
directory = os.getcwd()
for filename in os.listdir(directory):
    if filename.endswith('.txt'):
        ...
        xy = measure[["Label", "X", "Y"]]
        allXY = allXY.append(xy)


#view combined file - should have 22544 rows
allXY


#create a dictionary with the key as the image number and the list as the X and Y coordinates of the points
Dict = {}
for row in allXY.itertuples():
    image = row[1]
    #print image
    keyList = image.split(".")
    key = keyList[0][-4] + keyList[0][-3] + keyList[0][-2] + keyList[0][-1]
    #print key
    if key in Dict:
        #print row[2]
        #print row[3]
...
xyDFt.to_csv('Calculations_Nectary_allImages_CORRECT_200428.csv', sep=',', encoding='utf-8', index=False)
